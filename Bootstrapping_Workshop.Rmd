...
library(ggplot2)
# library(magrittr)
source("/Users/mcneill/Documents/Programming/R_Programming/GitHub/Analysis_Tools/Confidence_Intervals/bca_nonparametric_bootstrap_univariate_confidence_intervals.R")
...
```{r cumulative_distribution_functions}

# Assume true normal distribution with parameters mean 20, sd 5 (var 25)

# normal_sample <- tibble::tibble(sample_size = rep(c(10, 25, 50, 75, 100, 1e3, 1e4), 
#                                                   c(10, 25, 50, 75, 100, 1e3, 1e4)), 
#                                 sample_group = rep(c("A", "B", "C", "D", "E", "F", "G"), 
#                                                   c(10, 25, 50, 75, 100, 1e3, 1e4)), 
#                                 measurements = c(rnorm(n = 10, mean = 20, sd = 5), 
#                                                  rnorm(n = 25, mean = 20, sd = 5), 
#                                                  rnorm(n = 50, mean = 20, sd = 5), 
#                                                  rnorm(n = 75, mean = 20, sd = 5), 
#                                                  rnorm(n = 100, mean = 20, sd = 5), 
#                                                  rnorm(n = 1e3, mean = 20, sd = 5), 
#                                                  rnorm(n = 1e4, mean = 20, sd = 5)))
# 
# normal_sample <- dplyr::mutate(normal_sample, 
#                                sample_group = as.factor(sample_size))
# 
# save(normal_sample, file = "/Users/mcneill/Documents/Programming/R_Programming/GitHub/Bootstrapping_Workshop/Sample_from_normal_distribution.RData")
load(file = "/Users/mcneill/Documents/Programming/R_Programming/GitHub/Bootstrapping_Workshop/Sample_from_normal_distribution.RData")

ggplot(normal_sample, aes(x = measurements)) + 
  stat_function(fun = pnorm, args = list(mean = 20, sd = 5), color = "steelblue") + 
  facet_wrap("sample_size") + 
  stat_ecdf(geom = "step") + 
  labs(title = "Cumulative Distribution Function: Normal (20, 25)", 
       x = "Measurement", 
       y = "Probability")

ggplot(normal_sample, aes(x = measurements)) + 
  stat_density(alpha = 0.35) + 
  stat_function(fun = dnorm, args = list(mean = 20, sd = 5), color = "steelblue") + 
  facet_wrap("sample_size") + 
  labs(title = "Probability Density Function: Normal (20, 25)", 
       x = "Measurement", 
       y = "Density")

standard_estimates <- dplyr::summarise(dplyr::group_by(normal_sample, sample_group), 
                                       sample_size = dplyr::n(), 
                                       mean_est = mean(measurements), 
                                       median_est = median(measurements), 
                                       sd_unb_est = sd(measurements), 
                                       var_unb_est = var(measurements), 
                                       sd_mle_est = sd(measurements) * (sample_size - 1) / sample_size, 
                                       var_mle_est = var(measurements) * (sample_size - 1) / sample_size, 
                                       se_mean_unb_est = sqrt(var_unb_est / sample_size), 
                                       se_mean_mle_est = sqrt(var_mle_est / sample_size), 
                                       true_se_mean = sqrt(25 / sample_size), 
                                       true_var_var_unb = (5 * sample_size - 3) * 5 ^ 4 / (sample_size * (sample_size - 1)), 
                                       true_var_var_mle = (sample_size - 1) * (5 * sample_size - 3) * 5 ^ 4 / sample_size ^ 3, 
                                       true_bias_var_mle = - 5 ^ 2 / sample_size)

standard_estimates <- dplyr::mutate(standard_estimates, 
                                    mse_mean_unb_est = se_mean_unb_est ^ 2, 
                                    mse_mean_mle_est = se_mean_mle_est ^ 2, 
                                    true_mse_mean = true_se_mean ^ 2, 
                                    true_mse_var_unb = true_var_var_unb, 
                                    true_mse_var_mle = true_var_var_mle + true_bias_var_mle ^ 2)
...
```{r point_estimate_table}

knitr::kable(dplyr::select(standard_estimates, sample_size, mean_est, var_unb_est, var_mle_est),
             col.names = c("Sample Size",
                           "Sample Mean",
                           "Sample Variance (UNB)",
                           "Sample Variance (MLE)"),
             caption = "Sample Estimates on Normal(20, 25) Data",
             align = "llll",
             digits = 3,
...
```{r parametric_bootstrap_bias_estimation}

# The sample mean is an unbiased estimator for the true mean so we focus on the
# maximum likelihood sample variance  estimator for the variance

sample_var_mle <- function(x) {
  sum((x - mean(x))^2) / length(x)
}

# resampling_sample_var_mle <- function(resampling_vector, x) {
#   ordered_x <- x[order(x)]
#   mean_x <- mean(x)
#   
#   sum((ordered_x - mean_x)^2 * resampling_vector)
# }
# 
# parametric_normal_boot_bias_var_resampler <- function(Data_DF, statistic, B = 2e2) {
#   sample_groups <- unique(Data_DF$sample_group)
#   num_groups <- length(sample_groups)
#   
#   mle_estimates <- dplyr::summarise(dplyr::group_by(Data_DF, sample_group), 
#                                     sample_size = dplyr::n(), 
#                                     mean_est = mean(measurements), 
#                                     sd_est = sd(measurements))
#   
#   bootstrap_replicates <- tibble::tibble(sample_group = rep(sample_groups, rep(B, num_groups)))
#   bootstrap_reps <- list()
#   bootstrap_bias_ests <- list()
#   bootstrap_var_ests <- list()
#   bootstrap_bias_var <- tibble::tibble(sample_group = sample_groups)
#   
#   for (i in seq_along(sample_groups)) {
#     subset_estimates <- dplyr::filter(mle_estimates, sample_group == sample_groups[i])
#     subset_data <- dplyr::filter(Data_DF, sample_group == sample_groups[i])
#     ordered_measurements <- subset_data$measurements[order(subset_data$measurements)]
#     base_resample_vector <- rep(1 / subset_estimates$sample_size, length(ordered_measurements))
#     resample_vectors <- rmultinom(n = B, size = subset_estimates$sample_size, prob = base_resample_vector)
#     mean_resample_vector <- apply(resample_vectors, 1, FUN = mean) / (B * subset_estimates$sample_size)
#     stat_estimate <- statistic(mean_resample_vector, x = ordered_measurements)
#     bootstrap_estimates <- apply(resample_vectors, 2, x = ordered_measurements, FUN = statistic)
#     # boot_matrix <- matrix(bootstrap_samples, nrow = B, ncol = subset_estimates$sample_size, byrow = TRUE)
#     # bootstrap_estimates <- apply(boot_matrix, 1, FUN = statistic)
#     bootstrap_reps[[i]] <- bootstrap_estimates
#     bootstrap_bias_ests[[i]] <- mean(bootstrap_estimates) - stat_estimate
#     bootstrap_var_ests[[i]] <- var(bootstrap_estimates) # unbiased bootstrap var
#   }
#   
#   bootstrap_bias_var <- dplyr::mutate(bootstrap_bias_var, 
#                                       est_bias = unlist(bootstrap_bias_ests), 
#                                       est_var = unlist(bootstrap_var_ests), 
#                                       est_se = sqrt(est_var))
#   
#   bootstrap_replicates <- dplyr::mutate(bootstrap_replicates, 
#                                         bootstrap_statistic = unlist(bootstrap_reps))
#   
#   list(boot_par_ests = bootstrap_bias_var, replications = bootstrap_replicates, mle_estimates = mle_estimates)
# }
# 
# test <- parametric_normal_boot_bias_var_resampler(Data_DF = normal_sample, statistic = resampling_sample_var_mle, B= 2e2)





parametric_normal_boot_bias_var <- function(Data_DF, statistic, B = 2e2) {
  sample_groups <- unique(Data_DF$sample_group)
  num_groups <- length(sample_groups)
  ...
  for (i in seq_along(sample_groups)) {
    subset_estimates <- dplyr::filter(mle_estimates, sample_group == sample_groups[i])
    subset_data <- dplyr::filter(Data_DF, sample_group == sample_groups[i])
    stat_estimate <- statistic(subset_data$measurements)
    bootstrap_samples <- rnorm(n = subset_estimates$sample_size * B, mean = subset_estimates$mean_est, sd = subset_estimates$sd_est)
    boot_matrix <- matrix(bootstrap_samples, nrow = B, ncol = subset_estimates$sample_size, byrow = TRUE)
    bootstrap_estimates <- apply(boot_matrix, 1, FUN = statistic)
    bootstrap_reps[[i]] <- bootstrap_estimates
    bootstrap_bias_ests[[i]] <- mean(bootstrap_estimates) - stat_estimate
...
boot_bias_var <- function(Data_DF, statistic, B = 2e2) {
  sample_groups <- unique(Data_DF$sample_group)
  num_groups <- length(sample_groups)
  ...
  for (i in seq_along(sample_groups)) {
    ...
    stat_estimate <- statistic(subset_data$measurements)
    bootstrap_samples <- sample(subset_data$measurements, size = sample_size * B, replace = TRUE)
    boot_matrix <- matrix(bootstrap_samples, nrow = B, ncol = sample_size, byrow = TRUE)
    bootstrap_estimates <- apply(boot_matrix, 1, FUN = statistic)
    bootstrap_reps[[i]] <- bootstrap_estimates
    bootstrap_bias_ests[[i]] <- mean(bootstrap_estimates) - stat_estimate
    bootstrap_var_ests[[i]] <- var(bootstrap_estimates) # unbiased bootstrap var
  ...
  list(boot_par_ests = bootstrap_bias_var, replications = bootstrap_replicates)
}

# nonparametric_normal_bootstrap_200 <- boot_bias_var(Data_DF = normal_sample, statistic = sample_var_mle, B= 2e2)
# parametric_normal_bootstrap_200 <- parametric_normal_boot_bias_var(Data_DF = normal_sample, statistic = sample_var_mle, B= 2e2)
# nonparametric_normal_bootstrap_2000 <- boot_bias_var(Data_DF = normal_sample, statistic = sample_var_mle, B= 2e3)
# parametric_normal_bootstrap_2000 <- parametric_normal_boot_bias_var(Data_DF = normal_sample, statistic = sample_var_mle, B= 2e3)
# save(nonparametric_normal_bootstrap_200, parametric_normal_bootstrap_200, nonparametric_normal_bootstrap_2000, parametric_normal_bootstrap_2000, file = "/Users/mcneill/Documents/Programming/R_Programming/GitHub/Bootstrapping_Workshop/nonparametric_bootstrap_normal_example_data.RData")
load(file = "/Users/mcneill/Documents/Programming/R_Programming/GitHub/Bootstrapping_Workshop/nonparametric_bootstrap_normal_example_data.RData")

# standard_estimates <- dplyr::left_join(standard_estimates, parametric_normal_bootstrap$boot_par_ests, by = "sample_group")

normal_bootstrap_ests <- dplyr::bind_rows(parametric_normal_bootstrap_200$boot_par_ests, 
                                          nonparametric_normal_bootstrap_200$boot_par_ests, 
                                          parametric_normal_bootstrap_2000$boot_par_ests, 
                                          nonparametric_normal_bootstrap_2000$boot_par_ests)

normal_bootstrap_ests <- dplyr::mutate(normal_bootstrap_ests, 
                                       type = rep(c("parametric 200", 
                                                    "nonparametric 200", 
                                                    "parametric 2000", 
                                                    "nonparametric 2000"), rep(7, 4)))

selected_true_ests <- dplyr::select(standard_estimates, 
                                    true_bias_var_mle, 
                                    true_var_var_mle)

selected_true_ests <- dplyr::mutate(selected_true_ests, 
                                    true_se_var_mle = sqrt(true_var_var_mle))

selected_true_ests <- dplyr::bind_rows(selected_true_ests, 
                                       selected_true_ests, 
                                       selected_true_ests, 
                                       selected_true_ests)

# selected_true_ests <- dplyr::rename(selected_true_ests, c(est_bias = true_bias_var_mle, est_var = true_var_var_mle, est_se = true_se_var_mle))

normal_bootstrap_ests <- dplyr::bind_cols(normal_bootstrap_ests, selected_true_ests)

# normal_bootstrap_bias_var_ests_wide <- tidyr::pivot_wider(dplyr::select(normal_bootstrap_ests, sample_group, type, est_bias), names_from = type, values_from = est_bias)
# normal_bootstrap_se_var_ests_wide <- tidyr::pivot_wider(dplyr::select(normal_bootstrap_ests, sample_group, type, est_se), names_from = type, values_from = est_se)

ggplot(normal_bootstrap_ests, aes(x = true_bias_var_mle, y = est_bias, color = sample_group)) + 
  facet_wrap(~type) + 
  geom_abline(slope = 1, intercept = 0) + 
  geom_point() + 
  labs(title = "Scatterplot of Bootstrap Estimates of MLE Variance Estimator Bias", 
       subtitle = "From samples of normal (20, 25) distribution", 
       x = "True Theoretical Bias", 
       y = "Estimated Bias", 
       color = "Sample Size") + 
  scale_color_discrete(breaks = c("A", "B", "C", "D", "E", "F", "G"), 
                      labels = c(10, 25, 50, 75, 100, 1000, 10000))

ggplot(normal_bootstrap_ests, aes(x = true_se_var_mle, y = est_se, color = sample_group)) + 
  facet_wrap(~type) + 
  geom_abline(slope = 1, intercept = 0) + 
  geom_point() + 
  labs(title = "Scatterplot of Bootstrap Estimates of MLE Variance Estimator SE", 
       subtitle = "From samples of normal (20, 25) distribution", 
       x = "True Theoretical SE", 
       y = "Estimated SE", 
       color = "Sample Size") + 
  scale_color_discrete(breaks = c("A", "B", "C", "D", "E", "F", "G"), 
                      labels = c(10, 25, 50, 75, 100, 1000, 10000))

normal_bootstrap_replicate_data <- dplyr::bind_rows(parametric_normal_bootstrap_200$replications, 
                                                    nonparametric_normal_bootstrap_200$replications, 
                                                    parametric_normal_bootstrap_2000$replications, 
                                                    nonparametric_normal_bootstrap_2000$replications)

normal_bootstrap_replicate_data <- dplyr::mutate(normal_bootstrap_replicate_data, 
                                                 type = rep(c("Parametric 200", 
                                                              "Nonparametric 200", 
                                                              "Parametric 2000", 
                                                              "Nonparametric 2000"), 
                                                            c(1400, 1400, 14000, 14000)))

# ggplot(parametric_normal_bootstrap_200$replications, aes(x = bootstrap_statistic, fill = sample_group)) + 
#   geom_density(alpha = 0.4) + 
#   labs(title = "Density of Parametric Bootstrap Sample Variance Estimator", 
#        subtitle = "200 Bootstrap Replications Each", 
#        x = "MLE Variance", 
#        y = "Density", 
#        fill = "Sample Size") + 
...
#                       labels = c(10, 25, 50, 75, 100, 1000, 10000))
# 
# ggplot(nonparametric_normal_bootstrap_200$replications, aes(x = bootstrap_statistic, fill = sample_group)) + 
#   geom_density(alpha = 0.4) + 
#   labs(title = "Density of Nonparametric Bootstrap Sample Variance Estimator", 
#        subtitle = "200 Bootstrap Replications Each", 
#        x = "MLE Variance", 
#        y = "Density", 
#        fill = "Sample Size") + 
...
#                       labels = c(10, 25, 50, 75, 100, 1000, 10000))
# 
# ggplot(parametric_normal_bootstrap_2000$replications, aes(x = bootstrap_statistic, fill = sample_group)) + 
#   geom_density(alpha = 0.4) + 
#   labs(title = "Density of Parametric Bootstrap Sample Variance Estimator", 
#        subtitle = "2000 Bootstrap Replications Each", 
#        x = "MLE Variance", 
#        y = "Density", 
#        fill = "Sample Size") + 
#   scale_fill_discrete(breaks = c("A", "B", "C", "D", "E", "F", "G"), 
#                       labels = c(10, 25, 50, 75, 100, 1000, 10000))
# 
# ggplot(nonparametric_normal_bootstrap_2000$replications, aes(x = bootstrap_statistic, fill = sample_group)) + 
#   geom_density(alpha = 0.4) + 
#   labs(title = "Density of Nonparametric Bootstrap Sample Variance Estimator", 
#        subtitle = "2000 Bootstrap Replications Each", 
#        x = "MLE Variance", 
...
# View(nonparametric_normal_bootstrap_200$boot_par_ests)
# View(parametric_normal_bootstrap_200$boot_par_ests)
# View(nonparametric_normal_bootstrap_2000$boot_par_ests)
# View(parametric_normal_bootstrap_2000$boot_par_ests)
...
```{r bootstrap_estimate_comparison_table}

ss_vector <- c("A" = 10, "B" = 25, "C" = 50, "D" = 75, "E" = 100, "F" = 1e3, "G" = 1e4)

comparison_table <- dplyr::select(dplyr::mutate(normal_bootstrap_ests, 
                                                sample_size = ss_vector[sample_group]), 
                                  sample_size, 
                                  type, 
                                  true_bias_var_mle, 
                                  est_bias, 
...
knitr::kable(comparison_table,
             col.names = c("Sample Size",
                           ...
                           "True Standard Error", 
                           "SE Estimate"),
             caption = "Comparison of Bias and SE Estimates to True Values",
             align = "lllllll",
             digits = 3,
             format.args = list(scientific = FALSE))
...
```{r boostrap_replicate_densities}

ggplot(normal_bootstrap_replicate_data, aes(x = bootstrap_statistic, fill = sample_group)) + 
  facet_wrap(~type) + 
  geom_density(alpha = 0.4) + 
  labs(title = "Density of Parametric Bootstrap Sample Variance Estimator", 
       x = "MLE Variance", 
